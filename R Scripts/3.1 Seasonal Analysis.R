...
plotgeneral <- predict_gam(gamgeneral, exclude_terms = c("s(Year)","s(Site)"),series = "Month", length_out = 100, tran_fun = exp)%>%
  mutate(Behaviour = recode(Behaviour,
                            "Pairs" = "Paired visits", 
                            "MeanTimeSpentten" = "Mean visit \nduration(secs/10)","Visits" = "Total visits"))%>%
  plot(comparison = "Behaviour")+
  scale_x_continuous(breaks = 1:12, labels = month.abb[1:12])+
  scale_color_brewer(type = "qual") +
  scale_fill_brewer(type = "qual") +
  theme_minimal()+
  theme(legend.text = element_text(size = 25),  # Adjust legend text size
        legend.title = element_blank(),
        axis.title = element_text(size = 26),   # Adjust axis title size
        axis.text = element_text(size = 23),    # Adjust axis text size
        axis.text.x = element_text(size = 25, angle = 45, hjust =1),  # Adjust x-axis text size
...
plotresponse <- predict_gam(gamresponse, exclude_terms = c("s(Year)", "s(Site)"),series = "Month", length_out = 100, tran_fun = exp)%>%
  mutate(Behaviour = recode(Behaviour, 
                            "Responsesum" = "  Investigations\n (sniff) at site",
                            "Noresponsesum" = "No investigations\n (sniff) at site"))%>%
  plot(comparison = "Behaviour")+
  scale_color_brewer(type = "qual") +
  scale_fill_brewer(type = "qual") +
  scale_x_continuous(breaks = 1:12, labels = month.abb[1:12])+
...
plotbehaviour <- predict_gam(gambehaviour, exclude_terms = c("s(Year)", "s(Site)"),series = "Month", length_out = 100, tran_fun = exp)%>%
  mutate(Behaviour= recode(Behaviour,
                           "URsum"  = "Urinations",
                           "Rakesum" = "Rakes",
                           "Defsum" = "Defecations"))%>%
